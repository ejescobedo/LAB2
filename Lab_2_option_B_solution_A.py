...
#Edgar Escobedo
#80502432
#Lab 2 option B, Solution A
#***********************************************************************************************

#This lab is solution A, which uses the "iterative" traversal, meaning that every time a new node is introduced, the program
#will traverse the whole linked list in order to check if this is a new node and added, if it's already in the list
#it will simply increase the counter inside the node
...
#READING FILE
#creation of a list with all the usernames and passwords from the file
...
#the length is greater than 1 (because in some cases we have a null password), if it is, we will send the password as the information of
#the node and add it to our linked list
counter = 0
linked_list_passwords = Linked_List()
while (counter != len(passwords_list)):
    list_x = passwords_list[counter]
    wrds = list_x.split()
    if len(wrds) > 1:
        linked_list_passwords.add_to_list((wrds[1]))
    counter = counter + 1
...
second_list = linked_list_passwords
print("====================================== UNSORTED LINKED LIST ==========================================")
#Sorting the linked list
Linked_List.list_read(linked_list_passwords)
bubble_sort(linked_list_passwords)
print("================== SORTED TOP 20 PASSWORDS IN LINKED LIST USING BUBBLE SORT =========================")
Linked_List.list_20_top(linked_list_passwords)
